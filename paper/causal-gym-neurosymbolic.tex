...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % Gym Output
    \node[output, minimum width=2.5cm] (experience) at (2, -0.5) {Experience Buffer\\\texttt{experience\_buffer.csv}};
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % Models box
    %\node[rectangle, draw=purple!80, ultra thick, fill=white, minimum width=4cm, minimum height=5.5cm] (models-box) at (-2, 1) {};
    %\node[anchor=north west, font=\bfseries, text=purple!80] at (models-box.north west) {Outcome Predictor};
    
    \node[model, fill=purple!20] (rf-model) at (-2, 3) {\textbf{RandomForest}\\(or GradientBoosting)};
    \node[procstep, fill=gray!10] (predict-proba) at (-2, 1.5) {
      \textbf{Predict}\\
      $P(\text{success} | X)$
    };
    ...
    \draw[arrow, black!70] (case-features.south) -- ++(0,-0.3) -| node[pos=0.4, below, font=\tiny] {5. Features} (create-policy.south west);
    
    % Policy logic box
    \node[rectangle, draw=yellow!80, ultra thick, fill=white, minimum width=4.5cm, minimum height=2cm] (policy-box) at (8, 1) {};
    \node[anchor=north west, font=\bfseries, text=yellow!80] at (policy-box.north west) {Policy Induction Logic};
    
    \node[font=\tiny, align=left, text=black] at (8, 1) {
       \textbf{Strategy:}\\
    ...
    \node[procstep, fill=yellow!20] (optimal-action) at (8, 3) {
    ...
    \draw[arrow, black!70] (case-features.north) -- ++(0,2.5) -| node[pos=0.5, above, font=\tiny] {7. Features} (train-policy.north);
    
    % Policy model box
    %\node[rectangle, draw=green!80, ultra thick, fill=white, minimum width=4cm, minimum height=3cm] (final-box) at (17, 0) {};
    ...
    \draw[arrow, orange!70, ultra thick] (policy-predict.east) -- node[pos=0.7, left, font=\tiny\bfseries] {8. Export} (final-model-offline.west);
    ...
    % Strategy annotation
    \node[rectangle, draw=blue!60, fill=blue!10, rounded corners, align=left, font=\tiny, minimum width=5cm] at (-2, 0) {
        \textbf{Key Insight:}\\
